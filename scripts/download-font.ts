...
async function downloadFont() {
    ...
    if (!releaseResponse.ok) throw Error(`Could not fetch Nerd Fonts! Response status: ${releaseResponse.status}`);
    ...
    if (!releaseAsset) throw Error(`Could not find font archive '${ARCHIVE_NAME}' in latest release assets!`);
    ...
    if (!fontResponse.ok) throw Error(`Could not download requested font! Response status: ${fontResponse.status}`);
    ...
    if (!existsSync(FONT_PATH)) throw Error(`Could not find font file! Searching for: '${FONT_PATH}'`);
